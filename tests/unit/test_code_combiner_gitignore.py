...
def test_get_gitignore_spec_found_in_current_dir(mock_code_combiner_config):
    ...
    with patch.object(
        Path,
        "is_file",
        side_effect=lambda: (True if str(Path("/mock/dir/.gitignore")) == str(Path("/mock/dir/.gitignore")) else False),
    ):
        with patch(
            "builtins.open",
            MagicMock(return_value=MagicMock(__enter__=lambda self: ["*.pyc"], __exit__=MagicMock())),
        ):
...
def test_get_gitignore_spec_found_in_parent_dir(mock_code_combiner_config):
    ...
    mock_code_combiner_config.directory_path.resolve.return_value = Path("/mock/parent/dir")
    ...
    with patch.object(Path, "is_file", side_effect=is_file_side_effect):
        with patch(
            "builtins.open",
            MagicMock(return_value=MagicMock(__enter__=lambda self: ["*.log"], __exit__=MagicMock())),
        ):
            combiner = CodeMeld(mock_code_combiner_config)
            spec = combiner._get_gitignore_spec()
            assert spec is not None
