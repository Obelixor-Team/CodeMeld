...
def test_create_text_formatter_with_unknown_arg_raises_error():
    with pytest.raises(TypeError, match="Formatter 'text' initialization failed: Unknown arguments for text formatter: unknown_arg"):
...
def test_create_markdown_formatter_with_unknown_arg_raises_error():
    with pytest.raises(TypeError, match="Formatter 'markdown' initialization failed: Unknown arguments for markdown formatter: unknown_arg"):
        FormatterFactory.create("markdown", unknown_arg="value")

def test_create_json_formatter_with_unknown_arg_raises_error():
    with pytest.raises(TypeError, match="Formatter 'json' initialization failed: Unknown arguments for json formatter: unknown_arg"):
        FormatterFactory.create("json", unknown_arg="value")

def test_create_xml_formatter_with_unknown_arg_raises_error():
    with pytest.raises(TypeError, match="Formatter 'xml' initialization failed: Unknown arguments for xml formatter: unknown_arg"):
...
def test_formatter_factory_plugin_loading(mock_entry_points):
    class PluginFormatter(OutputFormatter):
        format_name = "plugin"
        def format_file(self, relative_path: Path, content: str) -> str: return ""
        def begin_output(self) -> str: return ""
        def end_output(self) -> str: return ""
