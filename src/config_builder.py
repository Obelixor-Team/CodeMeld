...
from .config import CombinerConfig, DEFAULT_EXTENSIONS, CodeCombinerError, ConvertType
...
def load_config_from_pyproject(root_path: Path) -> dict[str, Any]:
    ...
    if pyproject_path.is_file():
        try:
            ...
            # logging.warning(f"Could not load pyproject.toml: {e}") # Removed logging for now
            pass
    return config
